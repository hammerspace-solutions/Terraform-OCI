#!/bin/bash
#
# Ansible Job: Add ECGroup Volumes
#
# This script adds non-reserved logical volumes from ECGroup nodes only.
# It is idempotent and only adds new volumes when ecgroup_add_to_hammerspace=true and add_ecgroup_volumes=true.
...
STATE_FILE="/var/run/ansible_jobs_status/added_ecgroup_volumes.txt" # Track added ECGroup volume names
...
# --- Main Logic ---
echo "--- Starting Add ECGroup Volumes Job ---"
...
# 2. Get the username, password, volume group, share name, and volume control flags from the inventory
...
# Early exit if ECGroup is not being added to Hammerspace
if [ "$ecgroup_add_to_hammerspace" != "true" ]; then
  echo "ECGroup integration with Hammerspace is DISABLED (ecgroup_add_to_hammerspace=$ecgroup_add_to_hammerspace). Exiting."
  exit 0
fi

# Early exit if ECGroup volume addition is disabled
if [ "$add_ecgroup_volumes" != "true" ]; then
  echo "ECGroup volume addition is DISABLED (add_ecgroup_volumes=$add_ecgroup_volumes). Exiting."
  exit 0
fi
...
echo "Parsed add_storage_server_volumes: $add_storage_server_volumes"
echo "Parsed add_ecgroup_volumes: $add_ecgroup_volumes"
...
- hosts: localhost
  ...
  tasks:
    ...
    - name: Create ECGroup volumes for addition (filter out storage servers and existing)
      set_fact:
        volumes_for_add_json: |
          [
            ...
            {% for item in non_reserved_volumes %}
              ...
              {% if full_name not in existing_volumes and is_ecgroup %}
                {{ comma() }}
                {
